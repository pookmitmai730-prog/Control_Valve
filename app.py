...
# --- 2. การเชื่อมต่อ Firebase (ฉบับแก้ไขปัญหา Invalid Signature) ---
if not firebase_admin._apps:
    try:
        # ดึงข้อมูลจาก st.secrets
        fb_dict = dict(st.secrets["firebase"])
        
        # ล้างช่องว่างที่อาจติดมาจากการ Paste
        p_key = fb_dict["private_key"].strip()
        
        # แก้ไขปัญหาอักขระ \n ที่เป็น string ให้กลายเป็นขึ้นบรรทัดใหม่จริง
        # และจัดการกรณีรหัสผ่านถูกมองว่าเป็นบรรทัดเดียว
        if "\\n" in p_key:
            p_key = p_key.replace("\\n", "\n")
        
        # สำคัญ: ลบช่องว่างส่วนเกินในแต่ละบรรทัด (ถ้ามี)
        ...
        st.info("💡 คำแนะนำ: ตรวจสอบความถูกต้องของ Private Key ในหน้า Secrets อีกครั้ง")
        st.stop()
